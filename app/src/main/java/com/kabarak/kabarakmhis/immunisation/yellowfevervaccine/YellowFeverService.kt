...
import kotlinx.android.synthetic.main.activity_yellow_fever_service.*
import kotlinx.android.synthetic.main.activity_yellow_fever_service.navigation
import kotlinx.android.synthetic.main.activity_yellow_fever_service.tvAncId
import kotlinx.android.synthetic.main.activity_yellow_fever_service.tvPatient
...
class YellowFeverService : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_yellow_fever_service)
    ...
    private fun saveData() {
        ...
        if (tdProvided != ""){
            addData("Was Yellow Fever Immunization provided",tdProvided, DbObservationValues.YF_PROVIDED.name)

            if (linearYFYes.visibility == View.VISIBLE){

                val ttImmunization = tvYFDate.text. toString()
                if (!TextUtils.isEmpty(ttImmunization)){
                    addData("Immunization Date",ttImmunization, DbObservationValues.YF_RESULTS.name)
                }else{
        ...
        for (items in observationList){
            ...
            val data = DbDataList(key, value, DbSummaryTitle.A_YELLOW_FEVER.name, DbResourceType.Observation.name , label)
            dbDataList.add(data)
        ...
        if(errorList.isEmpty()){
            ...
            val dbPatientData = DbPatientData(DbResourceViews.YELLOW_FEVER.name, dbDataDetailsList)
            kabarakViewModel.insertInfo(this, dbPatientData)

            formatter.saveSharedPreference(this, "pageConfirmDetails", DbResourceViews.YELLOW_FEVER.name)
    ...
    private fun getData() {
        ...
        try {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                val encounterId = formatter.retrieveSharedPreference(this@YellowFeverService, DbResourceViews.YELLOW_FEVER.name)
                if (encounterId != null) {
                    val ttImmunization = patientDetailsViewModel.getObservationsPerCodeFromEncounter(
                        formatter.getCodes(DbObservationValues.YF_PROVIDED.name), encounterId
                    )
                    ...
                    withContext(Dispatchers.Main) {
                        if (ttImmunization.isNotEmpty()) {
                            ...
                            when {
                                ttImmunizationValue.contains("Yes", ignoreCase = true) -> radioGrpTD.check(R.id.radioBtnYes)
                                ttImmunizationValue.contains("No", ignoreCase = true) -> radioGrpTD.check(R.id.radioBtnNo)
                            }
                        }
                    }
                }
