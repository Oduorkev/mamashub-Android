...
class OtherProblemsReported() : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_other_problems_add)
        ...
        supportFragmentManager.setFragmentResultListener(
            ...
            Log.d("OtherProblems", "Submit request received")
            submitQuestionnaire()
        }
    }
            ...
            retrofitCallsFhir.submitQuestionnaireResponse(questionnaireResponseString, object :
                ...
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful) {
                        Toast.makeText(this@OtherProblemsReported, "Successfully submitted!", Toast.LENGTH_SHORT).show()
                        Log.d("OtherProblems", "Successfully submitted the questionnaire response.")
                        finish()
                    } else {
                        Toast.makeText(this@OtherProblemsReported, "Submission failed: ${response.message()}", Toast.LENGTH_SHORT).show()
                        Log.e("Error", "Failed to submit. Response code: ${response.code()}")
                    }
                }

                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    Toast.makeText(this@OtherProblemsReported, "Error occurred while submitting: ${t.message}", Toast.LENGTH_SHORT).show()
