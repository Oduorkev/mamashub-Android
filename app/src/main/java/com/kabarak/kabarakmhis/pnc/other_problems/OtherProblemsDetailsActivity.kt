...
import com.kabarak.kabarakmhis.pnc.other_problems.OtherProblemsEdit
...
class OtherProblemsDetailsActivity : AppCompatActivity() {

    private lateinit var retrofitCallsFhir: RetrofitCallsFhir
    private lateinit var recyclerView: RecyclerView
    private lateinit var adapter: OtherProblemsDetailsAdapter
    private val detailsList = mutableListOf<Detail>()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_other_problems_details)

        retrofitCallsFhir = RetrofitCallsFhir()
        recyclerView = findViewById(R.id.detailsRecyclerView)
        recyclerView.layoutManager = LinearLayoutManager(this)
        adapter = OtherProblemsDetailsAdapter(detailsList)
        recyclerView.adapter = adapter

        val responseId = intent.getStringExtra("responseId") ?: ""
        fetchOtherProblemsDetails(responseId)
    ...
    private fun fetchOtherProblemsDetails(responseId: String) {
    ...
    private fun parseQuestionnaireResponse(jsonResponse: String) {
        try {
            ...
            Log.e("OtherProblemsDetailsActivity", "Error parsing response", e)
        }
    }
