...
class BroadAdd : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportFragmentManager.setFragmentResultListener(
            ...
            Log.d("BroadAdd", "Submit request received")
            submitQuestionnaire()
        }
    }
            ...
            retrofitCallsFhir.submitQuestionnaireResponse(questionnaireResponseString, object : Callback<ResponseBody> {
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful) {
                        Toast.makeText(this@BroadAdd, "Successfully submitted!", Toast.LENGTH_SHORT).show()
                        Log.d("BroadAdd", "Successfully submitted the questionnaire response.")
                        finish()
                    } else {
                        Toast.makeText(this@BroadAdd, "Submission failed: ${response.message()}", Toast.LENGTH_SHORT).show()
                        Log.e("Error", "Failed to submit. Response code: ${response.code()}")
                    }
                }

                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    Toast.makeText(this@BroadAdd, "Error occurred while submitting: ${t.message}", Toast.LENGTH_SHORT).show()
