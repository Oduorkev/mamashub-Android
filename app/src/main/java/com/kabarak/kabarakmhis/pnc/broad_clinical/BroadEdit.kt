...
class BroadEdit : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        questionnaireJsonString = getStringFromAssets("broad-clinical.json")
        ...
        supportFragmentManager.setFragmentResultListener(
            ...
            Log.d("BroadEdit", "Submit request received")
    ...
    private suspend fun fetchAndPopulateQuestionnaireResponse(responseId: String) {
        ...
        retrofitCallsFhir.fetchQuestionnaireResponse(responseId, object : Callback<ResponseBody> {
                    ...
                    if (questionnaireResponseString != null) {
                        CoroutineScope(Dispatchers.Main).launch {
                            try {
                                ...
                                Log.e("BroadEdit", "Error populating questionnaire", e)
                                Toast.makeText(this@BroadEdit, "Error populating questionnaire", Toast.LENGTH_SHORT).show()
                            }
                        }
                    } else {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@BroadEdit, "Failed to retrieve the response data.", Toast.LENGTH_SHORT).show()
                        }
                    }
                } else {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@BroadEdit, "Failed to fetch the questionnaire response: ${response.message()}", Toast.LENGTH_SHORT).show()
                        Log.e("BroadEdit", "Failed to fetch response. Response code: ${response.code()}")
                    }
                }
            }
            ...
            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                CoroutineScope(Dispatchers.Main).launch {
                    Toast.makeText(this@BroadEdit, "Error occurred while fetching: ${t.message}", Toast.LENGTH_SHORT).show()
    ...
    private fun populateQuestionnaireFragment(questionnaireResponse: QuestionnaireResponse) {
        try {
            ...
            Log.d("BroadEdit", "Questionnaire response populated successfully.")
        } catch (e: Exception) {
            Log.e("BroadEdit", "Error initializing the questionnaire fragment or ViewModel", e)
        ...
        if (updatedQuestionnaireResponse != null) {
            ...
            retrofitCallsFhir.updateQuestionnaireResponse(responseId, updatedResponseString, object : Callback<ResponseBody> {
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful) {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@BroadEdit, "Data updated successfully.", Toast.LENGTH_SHORT).show()
                            // End the activity after successful submission
                            finish()
                        }
                    } else {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@BroadEdit, "Failed to update data: ${response.message()}", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
                ...
                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@BroadEdit, "Error occurred while updating: ${t.message}", Toast.LENGTH_SHORT).show()
                    }
                }
            })
        } else {
