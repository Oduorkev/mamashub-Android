...
class MeaslesImmunizationAddActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Load the Measles questionnaire JSON from assets
        questionnaireJsonString = readFileFromAssets("measlesvaccin.json")

        if (questionnaireJsonString == null) {
            Toast.makeText(this, "Failed to load Measles questionnaire.", Toast.LENGTH_SHORT).show()
            finish()
            return
        }
        ...
        viewModel.isImmunizationSaved.observe(this) { isSaved ->
            if (isSaved) {
                Toast.makeText(this, "Measles data saved successfully!", Toast.LENGTH_SHORT).show()
                finish()
            } else {
                Toast.makeText(this, "Failed to save Measles data.", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun handleSubmit() {
        ...
        if (fragment is QuestionnaireFragment) {
            ...
            Log.d("MeaslesImmunizationAddActivity", "QuestionnaireResponse (JSON): $jsonResponse")
            ...
            } ?: Log.e("MeaslesImmunizationAddActivity", "Questionnaire JSON is null")
        } else {
            Log.e("MeaslesImmunizationAddActivity", "QuestionnaireFragment not found")
        }
    }
}
