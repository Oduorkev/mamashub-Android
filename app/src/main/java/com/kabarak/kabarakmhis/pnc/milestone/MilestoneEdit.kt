...
class MilestoneEdit : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        supportFragmentManager.setFragmentResultListener(
            ...
            Log.d("MilestoneEdit", "Submit request received")
    ...
    private suspend fun fetchAndPopulateQuestionnaireResponse(responseId: String) {
        ...
        retrofitCallsFhir.fetchQuestionnaireResponse(responseId, object : Callback<ResponseBody> {
                    ...
                    if (questionnaireResponseString != null) {
                        CoroutineScope(Dispatchers.Main).launch {
                            try {
                                ...
                                Toast.makeText(this@MilestoneEdit, "Error populating questionnaire", Toast.LENGTH_SHORT).show()
                            }
                        }
                    } else {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@MilestoneEdit, "Failed to retrieve the response data.", Toast.LENGTH_SHORT).show()
                        }
                    }
                } else {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@MilestoneEdit, "Failed to fetch the questionnaire response: ${response.message()}", Toast.LENGTH_SHORT).show()
            ...
            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                CoroutineScope(Dispatchers.Main).launch {
                    Toast.makeText(this@MilestoneEdit, "Error occurred while fetching: ${t.message}", Toast.LENGTH_SHORT).show()
        ...
        if (updatedQuestionnaireResponse != null) {
            ...
            retrofitCallsFhir.updateQuestionnaireResponse(responseId, updatedResponseString, object : Callback<ResponseBody> {
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful) {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@MilestoneEdit, "Data updated successfully.", Toast.LENGTH_SHORT).show()
                            // End the activity after successful submission
                            finish()
                        }
                    } else {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@MilestoneEdit, "Failed to update data: ${response.message()}", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
                ...
                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@MilestoneEdit, "Error occurred while updating: ${t.message}", Toast.LENGTH_SHORT).show()
                    }
                }
            })
        } else {
