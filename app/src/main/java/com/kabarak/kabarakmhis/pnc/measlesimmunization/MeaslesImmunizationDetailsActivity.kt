...
class MeaslesImmunizationDetailsActivity : AppCompatActivity() {

    private lateinit var retrofitCallsFhir: RetrofitCallsFhir
    private lateinit var recyclerView: RecyclerView
    private lateinit var adapter: MeaslesImmunizationDetailsAdapter
    private val detailsList = mutableListOf<Detail>()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_measles_immunization_details)

        retrofitCallsFhir = RetrofitCallsFhir()
        recyclerView = findViewById(R.id.detailsRecyclerView)
        recyclerView.layoutManager = LinearLayoutManager(this)
        adapter = MeaslesImmunizationDetailsAdapter(detailsList)
        recyclerView.adapter = adapter

        val responseId = intent.getStringExtra("responseId") ?: ""
        fetchImmunizationDetails(responseId)

        val editButton: Button = findViewById(R.id.editButton)
        editButton.setOnClickListener {
            val intent = Intent(this, MeaslesImmunizationEditActivity::class.java)
            intent.putExtra("responseId", responseId)
            startActivity(intent)
        }
    }

    private fun fetchImmunizationDetails(responseId: String) {
    ...
    private fun parseQuestionnaireResponse(jsonResponse: String) {
        try {
            ...
            Log.e("MeaslesImmunizationDetailsActivity", "Error parsing response", e)
        }
    }
