...
class BcgVaccinationEditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_bcg_vaccination_edit)

        retrofitCallsFhir = RetrofitCallsFhir()

        questionnaireJsonString = getStringFromAssets("bcgpolio.json")
        ...
        supportFragmentManager.setFragmentResultListener(
            ...
            Log.d("BcgVaccinationEditActivity", "Submit request received")
    ...
    private fun getStringFromAssets(fileName: String): String? {
        return try {
            ...
            Log.e("BcgVaccinationEditActivity", "Error reading JSON from assets", e)
            null
        }
    }
    ...
    private suspend fun fetchAndPopulateQuestionnaireResponse(responseId: String) {
        retrofitCallsFhir.fetchQuestionnaireResponse(responseId, object : Callback<ResponseBody> {
                    ...
                    if (response.isSuccessful) {
                        response.body()?.string()?.let { questionnaireResponseString ->
                            try {
                                ...
                                Log.e("BcgVaccinationEditActivity", "Error parsing questionnaire response", e)
                                Toast.makeText(this@BcgVaccinationEditActivity, "Error populating questionnaire", Toast.LENGTH_SHORT).show()
                            }
                        } ?: run {
                            Toast.makeText(this@BcgVaccinationEditActivity, "Failed to retrieve the response data.", Toast.LENGTH_SHORT).show()
                        }
                    } else {
                        Log.e("BcgVaccinationEditActivity", "Failed to fetch response. Response code: ${response.code()}")
                        Toast.makeText(this@BcgVaccinationEditActivity, "Failed to fetch the questionnaire response: ${response.message()}", Toast.LENGTH_SHORT).show()
                    }
                }
            }
            ...
            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                CoroutineScope(Dispatchers.Main).launch {
                    Log.e("BcgVaccinationEditActivity", "Error occurred while fetching questionnaire response", t)
                    Toast.makeText(this@BcgVaccinationEditActivity, "Error occurred while fetching: ${t.message}", Toast.LENGTH_SHORT).show()
                }
            }
        })
    }
    ...
    private fun populateQuestionnaireFragment(questionnaireResponse: QuestionnaireResponse) {
        try {
            ...
            Log.d("BcgVaccinationEditActivity", "Questionnaire response populated successfully.")
        } catch (e: Exception) {
            Log.e("BcgVaccinationEditActivity", "Error initializing the questionnaire fragment", e)
        ...
        if (fragment is QuestionnaireFragment) {
            ...
            retrofitCallsFhir.updateQuestionnaireResponse(responseId, questionnaireResponseString, object : Callback<ResponseBody> {
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    CoroutineScope(Dispatchers.Main).launch {
                        if (response.isSuccessful) {
                            Toast.makeText(this@BcgVaccinationEditActivity, "Successfully updated!", Toast.LENGTH_SHORT).show()
                            Log.d("BcgVaccinationEditActivity", "Successfully updated the questionnaire response.")
                        } else {
                            val errorBody = response.errorBody()?.string() ?: "No error body"
                            Log.e("BcgVaccinationEditActivity", "Failed to update. Response code: ${response.code()}, Body: $errorBody")
                            Toast.makeText(this@BcgVaccinationEditActivity, "Update failed: ${response.message()}", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
                ...
                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@BcgVaccinationEditActivity, "Error occurred while updating: ${t.message}", Toast.LENGTH_SHORT).show()
                        Log.e("BcgVaccinationEditActivity", "Error occurred while updating questionnaire response", t)
                    }
                }
            })
        } else {
