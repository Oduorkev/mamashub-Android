            ...
            retrofitCallsFhir.submitQuestionnaireResponse(questionnaireResponseString, object :
                ...
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful) {
                        Toast.makeText(this@DiphtheriaAdd, "Successfully submitted!", Toast.LENGTH_SHORT).show()
                        Log.d("FamilyPlanning", "Successfully submitted the questionnaire response.")
                        finish()
                    } else {
                        Toast.makeText(this@DiphtheriaAdd, "Submission failed: ${response.message()}", Toast.LENGTH_SHORT).show()
                        Log.e("Error", "Failed to submit. Response code: ${response.code()}")
                    }
                }

                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    Toast.makeText(this@DiphtheriaAdd, "Error occurred while submitting: ${t.message}", Toast.LENGTH_SHORT).show()
        ...
        lifecycleScope.launch {
            try {
                ...
                retrofitCallsFhir.submitExtractedBundle(bundleJson, object : Callback<ResponseBody> {
                    override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                        if (response.isSuccessful) {
                            Toast.makeText(this@DiphtheriaAdd, "Resources submitted successfully!", Toast.LENGTH_SHORT).show()
                            Log.d("ChildCivilRegistration", "Successfully submitted the extracted FHIR resources.")
                            finish()
                        } else {
                            Toast.makeText(this@DiphtheriaAdd, "Resource submission failed: ${response.message()}", Toast.LENGTH_SHORT).show()
                            Log.e("Error", "Resource submission failed. Response code: ${response.code()}")
                        }
                    }

                    override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                        Toast.makeText(this@DiphtheriaAdd, "Resource submission failed: ${t.message}", Toast.LENGTH_SHORT).show()
