...
class MeaslesImmunizationEditActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_measles_immunization_edit)

        retrofitCallsFhir = RetrofitCallsFhir()

        // Load the Measles immunization questionnaire JSON file
        questionnaireJsonString = getStringFromAssets("measles_immunization.json")
        ...
        supportFragmentManager.setFragmentResultListener(
            ...
            Log.d("MeaslesImmunizationEditActivity", "Submit request received")
    ...
    private fun getStringFromAssets(fileName: String): String? {
        return try {
            ...
            Log.e("MeaslesImmunizationEditActivity", "Error reading JSON from assets", e)
            null
        }
    }
    ...
    private suspend fun fetchAndPopulateQuestionnaireResponse(responseId: String) {
        retrofitCallsFhir.fetchQuestionnaireResponse(responseId, object : Callback<ResponseBody> {
                    ...
                    if (response.isSuccessful) {
                        response.body()?.string()?.let { questionnaireResponseString ->
                            try {
                                ...
                                Log.e("MeaslesImmunizationEditActivity", "Error parsing questionnaire response", e)
                                Toast.makeText(this@MeaslesImmunizationEditActivity, "Error populating questionnaire", Toast.LENGTH_SHORT).show()
                            }
                        } ?: run {
                            Toast.makeText(this@MeaslesImmunizationEditActivity, "Failed to retrieve the response data.", Toast.LENGTH_SHORT).show()
                        }
                    } else {
                        Log.e("MeaslesImmunizationEditActivity", "Failed to fetch response. Response code: ${response.code()}")
                        Toast.makeText(this@MeaslesImmunizationEditActivity, "Failed to fetch the questionnaire response: ${response.message()}", Toast.LENGTH_SHORT).show()
                    }
                }
            }
            ...
            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                CoroutineScope(Dispatchers.Main).launch {
                    Log.e("MeaslesImmunizationEditActivity", "Error occurred while fetching questionnaire response", t)
                    Toast.makeText(this@MeaslesImmunizationEditActivity, "Error occurred while fetching: ${t.message}", Toast.LENGTH_SHORT).show()
                }
            }
        })
    }
    ...
    private fun populateQuestionnaireFragment(questionnaireResponse: QuestionnaireResponse) {
        try {
            ...
            Log.d("MeaslesImmunizationEditActivity", "Questionnaire response populated successfully.")
        } catch (e: Exception) {
            Log.e("MeaslesImmunizationEditActivity", "Error initializing the questionnaire fragment", e)
        ...
        if (fragment is QuestionnaireFragment) {
            ...
            retrofitCallsFhir.updateQuestionnaireResponse(responseId, questionnaireResponseString, object : Callback<ResponseBody> {
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    CoroutineScope(Dispatchers.Main).launch {
                        if (response.isSuccessful) {
                            Toast.makeText(this@MeaslesImmunizationEditActivity, "Successfully updated!", Toast.LENGTH_SHORT).show()
                            Log.d("MeaslesImmunizationEditActivity", "Successfully updated the questionnaire response.")
                        } else {
                            val errorBody = response.errorBody()?.string() ?: "No error body"
                            Log.e("MeaslesImmunizationEditActivity", "Failed to update. Response code: ${response.code()}, Body: $errorBody")
                            Toast.makeText(this@MeaslesImmunizationEditActivity, "Update failed: ${response.message()}", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
                ...
                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@MeaslesImmunizationEditActivity, "Error occurred while updating: ${t.message}", Toast.LENGTH_SHORT).show()
                        Log.e("MeaslesImmunizationEditActivity", "Error occurred while updating questionnaire response", t)
                    }
                }
            })
        } else {
